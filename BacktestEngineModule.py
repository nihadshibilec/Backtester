...
class BacktestEngine:
    def __init__(self, market_data, stoploss_pct, target_pct, capital=100000, leverage=5, entry_type="buy", max_trades_per_day=2, intraday = True):
        """
        ...
        - intraday (bool): Type of trading (intraday or swing)
    ...
    def run_backtest(self):
        ...
        for date in tqdm(unique_dates, desc="Backtesting", ncols=150):
            ...
            for i, row in filtered_data.iterrows():
                ...
                # If neither stoploss nor target is reached
                if i == filtered_data.index.max() and self.intraday == True and trade_status == 'active':
                ...
                if trade_status == 'active':
                    if self.check_exit_condition(row, trade_id):
                        self.place_exit_order(current_datetime, trade_id)
                        trade_status = 'inactive'
                        trade_id += 1
                        daily_trade_count += 1
    ...
    def check_entry_condition(self, row):
        ...
        """
        rsi = row['RSI']
        if row.name > 0:  # Check if the current row is not the first row
            prev_rsi = self.market_data.loc[row.name - 1, 'RSI']  # Access the previous row's RSI value
            return (rsi >= 70) and (prev_rsi < 70)
        else:
            return False 
...
market_data = pd.read_csv("D:/Market Data/RELIANCE.csv")
market_data = market_data.head(50000)
RSI_Strategy = BacktestEngine(market_data, stoploss_pct=0.01, target_pct=0.01, capital=100000, leverage=5, entry_type="sell", max_trades_per_day=2, intraday=False)
order_log = RSI_Strategy.run_backtest()
print(order_log)
order_log.to_csv("trade_log_test2.csv")
